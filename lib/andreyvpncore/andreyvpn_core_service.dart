...
import 'package:andreyvpn/features/connection/model/connection_failure.dart';
...
import 'package:andreyvpn/andreyvpncore/generated/v2/hcommon/common.pb.dart';
import 'package:andreyvpn/andreyvpncore/generated/v2/hcore/hcore.pb.dart';
import 'package:andreyvpn/andreyvpncore/generated/v2/hcore/hcore_service.pbgrpc.dart';
import 'package:andreyvpn/andreyvpncore/init_signal.dart';
import 'package:andreyvpn/singbox/model/singbox_config_option.dart';
import 'package:andreyvpn/features/log/model/log_level.dart' as config_log_level;
import 'package:andreyvpn/singbox/model/core_status.dart';
import 'package:andreyvpn/singbox/model/warp_account.dart';

import 'package:andreyvpn/andreyvpncore/core_interface/core_interface_wrapper_stub.dart'
    if (dart.library.io) 'package:andreyvpn/andreyvpncore/core_interface/core_interface_wrapper.dart';
...
class AndreyVPNCoreService with InfraLogger {
  AndreyVPNCoreService(this.ref);
  final Ref ref;

  // CoreAndreyVPNCoreService() {}
  ...
  Future<void> init() async {
    await setup()
        ...
        .map((_) {
          loggy.info("AndreyVPN-core setup done");
  ...
  TaskEither<String, Unit> changeOptions(SingboxConfigOption options) {
    return TaskEither(() async {
      ...
      try {
        final res = await core.fgClient.changeAndreyVPNSettings(
          ChangeAndreyVPNSettingsRequest(andreyvpnSettingsJson: jsonEncode(options.toJson())),
        );
        if (res.messageType != MessageType.EMPTY) return left("${res.messageType} ${res.message}");
        await core.bgClient.changeAndreyVPNSettings(
          ChangeAndreyVPNSettingsRequest(andreyvpnSettingsJson: jsonEncode(options.toJson())),
        );
  ...
  TaskEither<ConnectionFailure, Unit> start(String path, String name, bool disableMemoryLimit) {
    return TaskEither(() async {
      ...
      //   await core.bgClient.changeAndreyVPNSettings(
      //     ChangeAndreyVPNSettingsRequest(
      //       andreyvpnSettingsJson: jsonEncode(latestOptions!.toJson()),
      //     ),
      //   );
      // }
