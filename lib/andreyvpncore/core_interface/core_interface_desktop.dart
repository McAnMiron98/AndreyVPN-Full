...
import 'package:andreyvpn/gen/andreyvpn_core_generated_bindings.dart';
import 'package:andreyvpn/andreyvpncore/core_interface/core_interface.dart';
import 'package:andreyvpn/andreyvpncore/core_interface/mtls_channel_cred.dart';
...
import 'package:andreyvpn/andreyvpncore/generated/v2/hello/hello.pb.dart';
import 'package:andreyvpn/andreyvpncore/generated/v2/hello/hello_service.pbgrpc.dart';
...
final _logger = Loggy('AndreyVPNCoreFFI');
...
class CoreInterfaceDesktop extends CoreInterface with InfraLogger {
  static final AndreyVPNCoreNativeLibrary _box = _gen();

  static AndreyVPNCoreNativeLibrary _gen() {
    String fullPath = "";
    if (Platform.environment.containsKey('FLUTTER_TEST')) {
      fullPath = "andreyvpn-core";
    }
    if (Platform.isWindows) {
      fullPath = p.join(fullPath, "andreyvpn-core.dll");
    } else if (Platform.isMacOS) {
      fullPath = p.join(fullPath, "andreyvpn-core.dylib");
    } else {
      fullPath = p.join(fullPath, "andreyvpn-core.so");
    }

    _logger.debug('andreyvpn-core native libs path: "$fullPath"');
    ...
    return AndreyVPNCoreNativeLibrary(lib);
  }
